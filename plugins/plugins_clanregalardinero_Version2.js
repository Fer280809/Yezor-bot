// Regala dinero del clan a un miembro específico
...
const handler = async (m, { args, mentionByTag, mentionByReply, user }) => {
  ...
  if (!target || isNaN(cantidad)) return m.reply('Uso: .clanregalardinero @usuario <cantidad>')
  if (clans[clan].dinero < cantidad) return m.reply('El clan no tiene suficiente dinero.')
  clans[clan].dinero -= cantidad
  // Aquí puedes aumentar el saldo del usuario si tienes sistema
  await saveClans(clans)
  m.reply(`Has regalado $${cantidad} a ${target}`)
}
handler.help = ['clanregalardinero @usuario <cantidad>']
handler.tags = ['clan']
handler.command = ['clanregalardinero']
