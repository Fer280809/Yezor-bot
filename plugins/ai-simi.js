...
const handler = async (m, {conn, text, command, args, usedPrefix}) => {
    ...
    for (let keyword of blockedKeywords) {
        if (textLower.includes(keyword)) {
            await m.react('❌')
            const errorMessages = [
                '✘ Error: yezor no puede procesar comandos administrativos.',
    ...
    for (let pattern of commandPatterns) {
        if (pattern.test(textLower)) {
            ...
            for (let keyword of blockedKeywords) {
                if (textLower.includes(keyword)) {
                    await m.react('❌')
                    await conn.reply(m.chat, '✘ Error: yezor no puede proporcionar comandos administrativos.', m)
                    return
                }
            }
        }
