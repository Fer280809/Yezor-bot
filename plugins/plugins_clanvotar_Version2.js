...
const handler = async (m, { args, user }) => {
  ...
  if (!clans[clan].votacion.opciones.includes(opcion)) return m.reply('Opción no válida.')
  clans[clan].votacion.votos[sender] = opcion
  await saveClans(clans)
  m.reply(`Voto registrado: ${opcion}`)
}
handler.help = ['clanvotar <opcion>']
handler.tags = ['clan']
handler.command = ['clanvotar']
