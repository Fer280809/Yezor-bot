// Cancela la votación activa en el clan
...
const handler = async (m, { user }) => {
  ...
  if (!isOwnerOrAdmin(sender, clans[clan])) return m.reply('Solo owner/admin puede cancelar.')
  if (!clans[clan].votacion) return m.reply('No hay votación activa.')
  delete clans[clan].votacion
  await saveClans(clans)
  m.reply('Votación cancelada.')
}
handler.help = ['clancancelarvotacion']
handler.tags = ['clan']
handler.command = ['clancancelarvotacion']
