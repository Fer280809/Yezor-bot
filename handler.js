// ═══════════════════════════════════════════════════════
// handler.js - YeZor-Bot Handler con Sistema de Idiomas
// ═══════════════════════════════════════════════════════
  ...
  try {
    ...
    try {
      ...
      if (user) {
        ...
        if (!("language" in user)) user.language = global.language || "es"
      ...
      if (chat) {
        ...
        if (!("language" in chat)) chat.language = global.language || "es"
    ...
    // Función de traducción con idioma del usuario
    m.t = (key, customLang = null) => {
      const lang = customLang || user?.language || chat?.language || global.language || "es"
      return global.t(key, lang)
    }
    ...
    const groupMetadata = m.isGroup ? { 
      ...
      ...(((conn.chats[m.chat]?.metadata || await this.groupMetadata(m.chat).catch(_ => null) || {}).participants) && { 
        participants: ((conn.chats[m.chat]?.metadata || await this.groupMetadata(m.chat).catch(_ => null) || {}).participants || []).map(p => ({ 
          ...p, id: p.jid, jid: p.jid, lid: p.lid 
        })) 
      }) 
    } : {}
        ...
        if (chat) {
          ...
          if (name !== "group-banchat.js" && chat?.isBanned && !isROwner) {
            if (!primaryBotId || primaryBotId === botId) {
              const aviso = `✦ ${m.t('bot.offline')}\n\n> ${m.t('errors.groupOnly')}\n> » *${usedPrefix}bot on*`.trim()
              await m.reply(aviso)
              return
            }
          }
...
global.dfail = (type, m, conn) => {
  const msg = {
    rowner: m.t('errors.ownerOnly'),
    premium: m.t('errors.premiumOnly'),
    group: m.t('errors.groupOnly'),
    admin: m.t('errors.adminOnly'),
    botAdmin: m.t('errors.botAdminOnly')
  }[type]
