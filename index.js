...
import { YezorJadiBot } from './plugins/sockets-serbot.js'
...
say('Made with love by Fernando', {
  font: 'console',
  align: 'center',
...
const connectionOptions = {
  ...
  mobile: MethodMobile,
  browser: ["YeZor-Bot", "Safari", "1.0.0"],
  auth: {
    creds: state.creds,
...
conn.isInit = false
conn.well = false
conn.logger.info(`[ ✦ ] YeZor-Bot Iniciado\n`)
...
async function connectionUpdate(update) {
  ...
  if (connection === "close") {
    if ([401, 440, 428, 405].includes(reason)) {
      console.log(chalk.red(`→ (${code}) › Cierra la sesión Principal.`))
    }
    console.log(chalk.yellow("→ Reconectando YeZor-Bot..."))
    await global.reloadHandler(true).catch(console.error)
  }
}
...
// YeZor JadiBot (Cambiado de yukiJadibts)
